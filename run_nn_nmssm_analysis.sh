    ...
    do
        if [ "$MASS" -lt 1001 ]; then
            ...
            do
                ...
                IN_JSON="${IN_JSON} /storage/gridka-nrg/jbechtel/gc_storage/nmssm_limits/2020_05_19/${ERA}/${CHANNEL}/${VARIABLE}/nmssm_${CHANNEL}_${VARIABLE}_${MASS}_${MASS_H2}_cmb.json"
            ...
            # compare_limits.py  /ceph/jbechtel/save/postprocessing/nmssm/limits/nmssm_${CHANNEL}_nmssm_discriminator_${MASS}_cmb.json:exp0:Title='#splitline{3D discriminator}{cut-based categories}',LineStyle=1,LineWidth=2,LineColor=1,MarkerSize=0 nmssm_${CHANNEL}_nmssm_discriminator_${MASS}_cmb.json:exp0:Title='#splitline{3D discriminator}{NN-based categories}',LineStyle=1,LineWidth=2,LineColor=2,MarkerSize=0 nmssm_${CHANNEL}_${CHANNEL}_max_score_${MASS}_cmb.json:exp0:Title='#splitline{NN score}{NN-based categories}',LineStyle=1,LineWidth=2,LineColor=8,MarkerSize=0 --cms-sub "Own Work" --title-right "35.9 fb^{-1} (2016, 13 TeV)" --y-axis-min 0.001 --y-axis-max 500.0 --show exp   --output plots/limits/${CHANNEL}/nmssm_${CHANNEL}_compare_${MASS} --logy --process "nmssm" --title-left ${CHANNEL}"   m_{H} = "${MASS}" GeV" --xmax 850.0  --logx
