...
OUTPUTDIR="output/datacards/${ERA}-${CHANNEL}-${VARIABLE}-smhtt-control"
...
logandrun ${CMSSW_BASE}/bin/slc7_amd64_gcc700/MorphingSMRun2Legacy \
    ...
    --era=$ERA \
    --output=${OUTPUTDIR} | tee output/log/datacard-control-${ERA}-${CHANNEL}-${VARIABLE}-${JETFAKES}-${EMBEDDING}.log
