...
INPUT="output/datacards/${ERA}-${CHANNEL}-${VARIABLE}-smhtt-control/cmb/125"
echo "[INFO] Add datacards to workspace from path "${INPUT}"."

#OUTPUT=${ERA}_workspace.root
if [[ ! -d output/workspaces ]]
then
    mkdir -p output/workspaces
fi
OUTPUT=${PWD}/output/workspaces/${ERA}-${CHANNEL}-${VARIABLE}-control-workspace.root
