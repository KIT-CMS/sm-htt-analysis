...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Directory containing et fake factor inputs."
    )
    parser.add_argument(
        ...
        type=str,
        help=
        "Directory containing mt fake factor inputs."
    )
    parser.add_argument(
        ...
        type=str,
        help=
        "Directory containing tt fake factor inputs."
    )
...
def apply_fake_factors(config):
    ...
    unc_shifts = { #documented in https://twiki.cern.ch/twiki/bin/viewauth/CMS/HiggsToTauTauJet2TauFakes
        "et" : ["ff_qcd_syst",
                "ff_qcd_dm0_njet0_stat",
                "ff_qcd_dm0_njet1_stat",
                "ff_qcd_dm1_njet0_stat",
                "ff_qcd_dm1_njet1_stat",
                "ff_w_syst",
                "ff_w_dm0_njet0_stat",
                "ff_w_dm0_njet1_stat",
                "ff_w_dm1_njet0_stat",
                "ff_w_dm1_njet1_stat",
                "ff_w_syst",
                "ff_tt_dm0_njet0_stat",
                "ff_tt_dm0_njet1_stat",
                "ff_tt_dm1_njet0_stat",
                "ff_tt_dm1_njet1_stat"],
        "mt" : ["ff_qcd_syst",
                "ff_qcd_dm0_njet0_stat",
                "ff_qcd_dm0_njet1_stat",
                "ff_qcd_dm1_njet0_stat",
    ...
    input_friend_file = ROOT.TFile(os.path.join(getattr(args, "%s_friend_directory"%channel), datafile), "READ")
    ...
    ff_file = ROOT.TFile.Open(getattr(args, "%s_fake_factor_directory"%channel))
    ff = ff_file.Get('ff_comb')
    
    #prepare output
    logger.debug("...initialize %s"%os.path.join(args.output_directory, datafile))
    ...
    for x in suffix[channel]:
        ...
        output_tree.Branch("ff%i_nom"%x, output_buffer["nom_%i"%x], "d0_1_calib/D")
        for syst in unc_shifts[channel]:
            for shift in ["up", "down"]:
                output_buffer["%s_%s_%i"%(syst, shift, x)] = numpy.zeros(1, dtype=float)
                output_tree.Branch("ff%i_%s_%s"%(x, syst, shift), output_buffer["%s_%s_%i"%(syst, shift, x)], "d0_1_calib/D")
    
    #fill tree
    for event in input_tree:
        for x in suffix[channel]:
            inputs = []
            cat_fractions = fractions[channel][categories[channel][int(getattr(event, "%s_max_index"%channel) + (0.5 * len(categories[channel]) if channel=="tt" and x==2 else 0.0))]]
            if channel == "tt":
                inputs = [getattr(event, "pt_%i"%x), getattr(event, "pt_%i"%(3-x)), getattr(event, "decayMode_%i"%x), event.njets, event.m_vis]
                inputs.extend((cat_fractions["QCD"], cat_fractions["W"], cat_fractions["TT"], cat_fractions["DY"]))
            else:
                inputs = [event.pt_2, event.decayMode_2, event.njets, event.m_vis, event.mt_1, event.iso_1]
                inputs.extend((cat_fractions["QCD"], cat_fractions["W"], cat_fractions["TT"]))
