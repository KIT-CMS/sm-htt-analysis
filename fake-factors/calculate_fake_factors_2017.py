...
def apply_fake_factors(config):
    ...
    if getattr(args, "%s_friend_directory" % channel) is not None:
    ...
    for event in input_tree:
        for x in suffix[channel]:
            inputs = []
            if args.config[channel]["expression"] == "m_vis":
                cat_fractions   = fractions[channel][categories[channel][0 if channel is not "tt" else int(x-1)]]
            elif args.config[channel]["expression"] == "NN_score":
...
def main(args):
    ...
    if args.config["mt"]["expression"] == "m_vis":
        categories = {
            "et": ["m_vis"],
            "mt": ["m_vis"],
            "tt": [
    ...
    elif args.config["mt"]["expression"] == "NN_score":
        categories = {
    ...
    for entry in os.listdir(args.directory):
        if "Run2017" in entry and ("Single" in entry
                                            or "Tau" in entry):
