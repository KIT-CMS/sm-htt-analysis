        ...
        for era in ${eras[@]}; do
            ...
            for channel in ${channels[@]}; do
                ...
                (
                    ...
                    for OPTION in "--png" ""
                    do
                        logandrun ./plotting/plot_shapes.py -i $FILE -o $PLOTDIR -c ${channel} -e $era $OPTION --png --categories $CATEGORIES --fake-factor --embedding --normalize-by-bin-width -l --train-ff False --train-emb False
                    done
                )
                done
        done
