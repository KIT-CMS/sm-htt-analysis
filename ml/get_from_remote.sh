...
if [[ ${_CONDOR_JOB_IWD} == "/var/lib/condor/execute/dir"* ]]; then
    ...
    # Name hardcoded as HTCondor does not preserve $USER
    USE_USER=tvoigtlaender
    cephpath=root://ceph-node-a.etp.kit.edu:1094//${USE_USER}/nmssm_data
else
    echo "Running locally"
    cephpath=/ceph/srv/${USE_USER}/nmssm_data
fi
...
sed -e 's@\(early_stopping: \).*$@\120@g' -i output/ml/${folder}/dataset_config.yaml
# sed -e 's@\(epochs: \).*$@\1100@g' -i output/ml/${folder}/dataset_config.yaml
