...
if [[ $(pwd) == "/var/lib/condor/execute/dir"* ]]; then
    echo "Running on HTCondor"
    cephpath=root://ceph-node-a.etp.kit.edu:1094//tvoigtlaender/nmssm_data
else
    echo "Running locally"
    cephpath=/ceph/srv/tvoigtlaender/nmssm_data
fi
...
# sed -e 's@\(name: \).*$@\1smhtt_dropout_tanh@g' -i output/ml/${folder}/dataset_config.yaml
