...
def create_properties(histname):
    ...
    return {"inputname" : histname, "channel" : channel, "category" : category, "process" : process, "outputname" : outputname, "variable" : variable, "era" : era.replace("Run","")}
...
def main(args):
    ...
    for p in properties_list:
        ...
        if not p["era"] in hist_map:
            hist_map[p["era"]] = {}
        if not p["channel"] in hist_map[p["era"]]:
            hist_map[p["era"]][p["channel"]] = {}
        if not p["variable"] in hist_map[p["era"]][p["channel"]]:
            hist_map[p["era"]][p["channel"]][p["variable"]] = {}
        if not p["category"] in hist_map[p["era"]][p["channel"]][p["variable"]]:
            hist_map[p["era"]][p["channel"]][p["variable"]][p["category"]] = {}
        hist_map[p["era"]][p["channel"]][p["variable"]][p["category"]][p["inputname"]] = p["outputname"]

    infolist =  []
    for era in hist_map:
        for channel in hist_map[era]:
            for variable in hist_map[era][channel]:
                for category in hist_map[era][channel][variable]:
                    infolist.append({"processes" : hist_map[era][channel][variable][category], "channel" : channel, "category": category, "era" : era, "variable" : variable, "input" : args.input, "path" : args.output})
...
if __name__ == "__main__":
    ...
    setup_logging("convert_synced_shapes_categories.log", logging.DEBUG)
    main(args)
