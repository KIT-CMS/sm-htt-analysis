...
if [ "$CHANNEL" == "all_em" ]
    then
    MorphingMSSMvsSM --era=${ERA} --auto_rebin=1 --binomial_bbb=1 --analysis=nmssm --channel="mt et tt em" --heavy_mass=${MASS}  --light_mass=${MASS_H2} --output_folder="output_"${ERA}"_"${CHANNEL}
else
...
if [ "$CHANNEL" == "all" ]
    ...
    MorphingMSSMvsSM --era=${ERA} --auto_rebin=1 --binomial_bbb=1 --analysis=nmssm --channel=${CHANNEL} --heavy_mass=${MASS} --light_mass=${MASS_H2} --output_folder="output_"${ERA}"_"${CHANNEL} --real_data=1
fi
fi
combineTool.py -M T2W -o "ws.root"  --PO '"map=^.*/NMSSM_'${MASS}'_125_'${MASS_H2}'$:r_NMSSM_'${MASS}'_125_'${MASS_H2}'[0,0,5]"' -i output_${ERA}_${CHANNEL}_nmssm_${MASS}_${MASS_H2}/${ERA}/cmb/ -m ${MASS_H2} --parallel 1 -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel

combineTool.py -m $MASS_H2 -M AsymptoticLimits --rAbsAcc 0 --rRelAcc 0.0005  --setParameters r_NMSSM_${MASS}_125_${MASS_H2}=0 --redefineSignalPOIs r_NMSSM_${MASS}_125_${MASS_H2} -d output_${ERA}_${CHANNEL}_nmssm_${MASS}_${MASS_H2}/${ERA}/cmb/ws.root --there -n ".NMSSM_"${MASS}"_125_"  --task-name NMSSM_${MASS}_125_${MASS_H2} --parallel 1 -t -1

combineTool.py -M CollectLimits output_${ERA}_${CHANNEL}_nmssm_${MASS}_${MASS_H2}/${ERA}/cmb/higgsCombine*.root --use-dirs -o nmssm_${CHANNEL}_${MASS}_${MASS_H2}.json

PostFitShapesFromWorkspace -d output_${ERA}_${CHANNEL}_nmssm_${MASS}_${MASS_H2}/${ERA}/cmb/combined.txt.cmb -w output_${ERA}_${CHANNEL}_nmssm_${MASS}_${MASS_H2}/${ERA}/cmb/ws.root -o output_${ERA}_${CHANNEL}_nmssm_${MASS}_${MASS_H2}/${ERA}/cmb/prefitshape.root --freeze r_NMSSM_${MASS}_125_${MASS_H2}=0.1
